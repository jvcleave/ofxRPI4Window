...
void ofApp::draw(){
    ...
    auto xPosition = ofGetFrameNum() % ofGetWidth();
    //ofLog() << "xPosition: " << xPosition;
    if(ofGetFrameNum() % 100 == 0)
    {
        if(currentKeyIndex + 1 < keys.size())
        {
            currentKeyIndex++;
        }else
        {
        ...
        keyPressed(keys[currentKeyIndex]);
    }
    
    ofSetColor(255);
    auto yPosition = ofGetFrameNum() % ofGetHeight();

    mouseDragged(xPosition, yPosition, 0);
